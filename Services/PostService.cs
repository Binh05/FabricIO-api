...
public class PostService(IUnitOfWork unitOfWork, IMapper mapper, IStorageServices storageServices) : IPostService
{
    public async Task<PostResponseDto> CreatePostAsync(Guid userId, PostRequestDto request, CancellationToken token)
    {
        ...
        if (request.MediaFiles != null && request.MediaFiles.Any())
        {
            ...
            {
                var mediaUrl = await storageServices.UploadAsync(file, token);
